export type MapToArrayResult<K extends keyof V, V extends object, P extends keyof V | readonly (keyof V)[] | undefined> = Record<K, V[K] & PropertyKey> &
  (P extends undefined ? V : P extends keyof V ? Record<P, V[P]> : P extends readonly (keyof V)[] ? Pick<V, P[number]> : never);

export function mapToArray<K extends keyof V, V extends object>(map: Map<K, V> | Record<K, V>, keyProp: K): (V & Record<K, V[K]>)[];
 ...
 * // ]
 */
export function mapToArray<K extends keyof V, V extends object, P extends keyof V | readonly (keyof V)[] | undefined = undefined>(
  map: Map<K, V> | Record<K, V>,
  keyProp: K,
  valueProps?: P
): MapToArrayResult<K, V, P>[] {
  const size = map instanceof Map ? map.size : undefined;
  const result: MapToArrayResult<K, V, P>[] = size !== undefined ? new Array(size) : [];

  let index = 0;

  const createMappedObject = (key: K, value: V): MapToArrayResult<K, V, P> => {
    if (valueProps === undefined) {
      return { [keyProp]: key, ...value } as MapToArrayResult<K, V, P>;
    }
    if (Array.isArray(valueProps)) {
      const propArray = valueProps as readonly (keyof V)[];
      const selectedProps = {} as Pick<V, (typeof propArray)[number]>;
      for (const prop of propArray) {
        selectedProps[prop] = value[prop];
      }
      return { [keyProp]: key, ...selectedProps } as MapToArrayResult<K, V, P>;
    }
    return { [keyProp]: key, [valueProps as keyof V]: value[valueProps as keyof V] } as MapToArrayResult<K, V, P>;
  };

  if (map instanceof Map) {
    for (const [key, value] of map) {
      result[index++] = createMappedObject(key, value);
    }
  } else {
    for (const key in map) {
      if (Object.prototype.hasOwnProperty.call(map, key)) {
        result[index++] = createMappedObject(key as K, map[key]);
      }
    }
  }
...
// export function multiMapToArray<T, K extends keyof T>(data: Map<T[K], T[]> | Record<T[K] & PropertyKey, T[]>, keyProp: K): T[];
//
...
// ): T[] {
//   const entries =
//     data instanceof Map ? Array.from(data.entries()) : (Object.entries(data) as [string, (V extends readonly (keyof T)[] ? Pick<T, V[number]> : T[Extract<V, keyof T>])[]][]);
